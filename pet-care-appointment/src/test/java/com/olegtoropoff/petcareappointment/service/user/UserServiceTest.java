...
class UserServiceTest {
    ...
    private Review createReview(Long reviewId, User patient, User veterinarian) {
    ...
    @Test
    void getUserWithDetails_WhenValidUserId_ReturnsUserDto() throws SQLException {
        Long userId = 1L;
        ...
        Review review1 = createReview(1L, patient, null);
        Review review2 = createReview(2L, null, veterinarian);

        Page<Review> reviewPage = new PageImpl<>(List.of(review1, review2));

        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        when(entityConverter.mapEntityToDto(user, UserDto.class)).thenReturn(userDto);
        when(reviewRepository.countByVeterinarianId(userId)).thenReturn(5L);
        when(appointmentService.getUserAppointments(userId)).thenReturn(Collections.emptyList());
        when(photoService.getImageData(2L)).thenReturn(new byte[]{1, 2, 3});
        when(reviewService.findAllReviewsByUserId(userId, 0, Integer.MAX_VALUE)).thenReturn(reviewPage);
        when(reviewService.getAverageRatingForVet(userId)).thenReturn(4.5);

        UserDto result = userService.getUserWithDetails(userId);

        assertNotNull(result);
        assertEquals(1L, result.getId());
        assertEquals(5L, result.getTotalReviewers());
        ...
        verify(photoService, times(1)).getImageData(2L);
    }
}
